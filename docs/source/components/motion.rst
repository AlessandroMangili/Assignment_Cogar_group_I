...
| The **motion** component is used to manage, as the name suggests, the robot's navigation through the **encoders**, **controllers**, and **motors** of both wheels. The **control** components will receive the path signal to follow from the module within the :doc:`Navigation<navigation>` along with the respective speeds to be set for the wheels.
